...
<View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
  ...
  return end <= start;
        ...
        >
                ...
                {/* BADGE OGGI / PROSSIMO SAFE */}
                {(() => {
                  const oggiBadge = new Date();
                  oggiBadge.setHours(0, 0, 0, 0);

                  const dataCardBadge = new Date(t.data);
                  dataCardBadge.setHours(0, 0, 0, 0);

                  const eOggiBadge =
                    dataCardBadge.getTime() === oggiBadge.getTime();

                  const prossimoBadge = ordinati
                    .filter(
                      (x) =>
                        x &&
                        x.tipo === 'turno' &&
                    ...
                    .filter(({ d }) => d > oggiBadge)
                    .sort((a, b) => a.d - b.d)[0]?.x;

                  const eProssimoBadge =
                    !eOggiBadge &&
                    t.tipo === 'turno' &&
                    prossimoBadge &&
                    t.id === prossimoBadge.id;

                  if (!eOggiBadge && !eProssimoBadge) {
                    return null;
                  }
                  ...
                  return (
                    <View
                      style={{
                        ...
                        paddingVertical: 4,
                        borderRadius: 10,
                        backgroundColor: eOggiBadge
                          ? '#67F29A'
                          : '#FFD166',
                        borderWidth: 1,
                        borderColor: eOggiBadge
                          ? '#B6FFCF'
                          : '#FFE6A6',
                        shadowColor: eOggiBadge
                          ? '#67F29A'
                          : '#FFD166',
                        shadowOpacity: 0.30,
                        shadowRadius: 7,
