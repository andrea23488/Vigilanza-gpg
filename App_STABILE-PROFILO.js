...
export default function App() {
  ...
  async function caricaTurni() {
    try {
      ...
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${testo}`);
      }
  ...
  async function scegliFotoProfilo() {
    try {
      ...
      Alert.alert(
        'Errore',
        'Non sono riuscito ad aprire la libreria foto.'
      );
    }
  }
    ...
    return (
      <Screen>
        ...
        <Text style={styles.profileNote}>
          In seguito collegheremo foto e profilo
          all'account personale, così resteranno
          sincronizzati anche cambiando dispositivo.
        </Text>
      </Screen>
    );
  }
