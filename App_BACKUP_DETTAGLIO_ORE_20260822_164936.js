...
export default function App() {
 ...
 const [accessoTest, setAccessoTest] = useState(false);
  ...
  async function caricaTurni(mostraLoading = true) {
    ...
    const riepilogoMesi = lista.reduce((acc, t) => {
      const chiave = `${String(t.mese).padStart(2, '0')}/${t.anno}`;
    ...
    Alert.alert(
      'Diagnostica turni',
      Object.entries(riepilogoMesi)
        .sort()
        .map(([meseAnno, quanti]) => `${meseAnno}: ${quanti} turni`)
    ...
    return (
      <Screen>
        ...
        >
          ...
          >
            Extra: {extraStipendioMese.toFixed(1)} h · Giorni lavorati: {giornateStipendioMese.length}
          </Text>
        </View>
